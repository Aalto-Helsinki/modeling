...
-enzyme business thingy, stop evaluating the sub list
when enzyme and sub bind
-no substrate can transform more than one time per timestep,
I think this can't happen now.
-more enzymes, done perhaps, adding is easy
-REFACTORING!!!!!! this shit is atrocious
-check the python interpreter, could cpython be faster?
...
def updateobj(obj, delta, dt):
    ...
    y = norm.rvs(scale=delta**2*dt)
    dpos = Vector2(x,y)
    if (dpos + obj.getPosition()).length() < 10:
